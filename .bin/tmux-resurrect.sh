#!/bin/bash
# found this here http://brainscraps.wikia.com/wiki/Resurrecting_tmux_Sessions_After_Reboot
# Edited the mimic_state to restore PWD from my weird zsh prompt
# I got this to run on mac using launchctl http://launched.zerowidth.com/
# <?xml version="1.0" encoding="UTF-8"?>
# <!DOCTYPE plist PUBLIC "-//Apple Computer//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
# <plist version="1.0">
# <dict>
# <key>Label</key>
# <string>com.johnjrussell.tmux-resurrect</string>
# <key>ProgramArguments</key>
# <array>
# <string>bash</string>
# <string>-c</string>
# <string>Dropbox/jjr-environment/bin/tmux-resurrect.sh -b</string>
# </array>
# <key>StartInterval</key>
# <integer>60</integer>
# <key>UserName</key>
# <string>jjrussell</string>
# <key>WorkingDirectory</key>
# <string>/Users/jjrussell</string>
# <key>StandardOutPath</key>
# <string>/tmp/resurrect.out</string>
...
mimic_state() {
    # local prompt_regex='^\<.*@.*:'
    # local prompt_line=$(grep $(eval echo ${prompt_regex}) ${backup_dir}/${pane_full} | tail -n 1)
    # local host=$(printf "${prompt_line}" | cut -d@ -f2 | cut -d: -f1)
    #local working_dir=$(printf "${prompt_line}" | cut -d: -f2 | cut -d\> -f1)
    local working_dir=$(grep "${USER}@" ${backup_dir}/${pane_full} | tail -1 | ruby -nae 'puts $F[2].gsub(/\[|\]/,"")')
...
#######################
# Code
function rotate_backup_directories(){
    i=${max_backups}
    while [[ ${i} != 0 ]] ;do
...
if [ "${task}" = 'backup' ] && pgrep -q tmux ; then
    ...
    for session in $(tmux list-sessions | awk -F: '{print $1}') ;do
        ...
        for window in $(tmux list-windows -t ${session} | awk -F: '{print $1}') ;do
            tmux list-window -t 0 | grep "^${window}" | ruby -nae 'print $F[1].gsub(/\*|-$/,"")' > ${backup_dir}/${session}:${window}.window_name
            for pane in $(tmux list-panes -t ${session}:${window} | awk -F: '{print $1}') ;do
                ...
                # If this happens while you're using tmux it messes with the history and is annoying.
    ...
    for session in $(ls ${backup_dir} | grep -v '\.size$' | cut -d: -f1 | sort -du) ;do
        ...
        for window in ${window_list} ;do
            window_name=$(cat ${backup_dir}/${session}:${window}.window_name)
            tmux rename-window -t ${session}:${window} "${window_name}"

            pane_list=$(ls ${backup_dir} | grep -v 'layout$' | grep -v 'window_name$' | awk -F\. '/^'${session}':'${window}'\./ {print $2}')
